...
class AquaIllumination(LightEntity):
    ...
    async def async_turn_on(self, **kwargs):
        ...
        # Fall back to current brightness if no brightness is provided in kwargs, otherwise default to max.
        target_brightness = kwargs.get(ATTR_BRIGHTNESS)
        if target_brightness is None:
            target_brightness = self._brightness if self._brightness else 255
